...
async def create_node_recursive(session:AsyncSession, story_id:int, base_prompt:str, path_id:int, parent_id:int):
    parent_node = await get_node(session=session, node_id=parent_id)
    options = [parent_node.option_one, parent_node.option_two, parent_node.option_one, parent_node.option_two]
    option = options[path_id]
    curr_storyline = parent_node.short_line
    short_line = generate_storyline(base_prompt, current_storyline=curr_storyline, option_text=option)
    image_prompt = generate_background(base_prompt, short_line)
    image = image_generate(image_prompt)
    character_name = generate_name(short_line)
    question = generate_question(base_prompt, short_line, image_prompt)
